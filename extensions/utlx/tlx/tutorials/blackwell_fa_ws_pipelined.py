...
def _softmax_inner_loop(
    ...
    for start_n in tl.range(lo, hi, BLOCK_N):
        ...
        tlx.barrier_wait(tlx.local_view(alpha_empties, qk_bufIdx),
                         qk_phase ^ 1)
        # Use alpha[0] for cid=0, and alpha[HEAD_DIM * NUM_BUFFERS_QK] for cid=1
        tlx.local_store(
            tlx.local_view(alpha_tiles, cid * HEAD_DIM * NUM_BUFFERS_QK),
            alpha[:, None])
    ...
    with tlx.async_tasks():
        ...
        with tlx.async_task("default"):
            ...
            for cid in tl.range(0,
                ...
                l = tlx.local_load(l_tiles[cid * HEAD_DIM * NUM_BUFFERS_QK +
                                           1])
                m = tlx.local_load(m_tiles[cid * HEAD_DIM * NUM_BUFFERS_QK +
                                           2])
                m += tl.math.log2(l)
        ...
        with tlx.async_task(num_warps=4,
            ...
            # Use l[1]/l[1+HEAD_DIM * NUM_BUFFERS_QK] and m[2][2 + HEAD_DIM * NUM_BUFFERS_QK]
            # to disambigulate from alpha[0]/alpha[HEAD_DIM * NUM_BUFFERS_QK]
            tlx.local_store(l_tiles[cid * HEAD_DIM * NUM_BUFFERS_QK + 1],
                            l_i[:, None])
            tlx.local_store(m_tiles[cid * HEAD_DIM * NUM_BUFFERS_QK + 2],
                            m_i[:, None])
        ...
        with tlx.async_task(num_warps=1, registers=24):
            ...
            tlx.barrier_expect_bytes(q_fulls[0],
                                     2 * BLOCK_M_SPLIT * HEAD_DIM)  # float16
            qo_offset_y_split = qo_offset_y
            tlx.async_descriptor_load(desc_q, q_tiles[0],
                                      [qo_offset_y_split, 0], q_fulls[0])
            ...
            # load q1
            tlx.barrier_expect_bytes(q_fulls[1],
                                     2 * BLOCK_M_SPLIT * HEAD_DIM)  # float16
            qo_offset_y_split = qo_offset_y + BLOCK_M_SPLIT
            tlx.async_descriptor_load(desc_q, q_tiles[1],
                                      [qo_offset_y_split, 0], q_fulls[1])
