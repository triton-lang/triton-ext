...
def get_cuda_autotune_config():
    if os.getenv("ENABLE_PROTON") == "1":
        return [
            ...
            for BN in [128] for bq in [1] for bkv in [3]
            for bqk in [1]  # in tmem
            for bo in [1]  # in tmem
            for SUBTILE in [True]  # doesn't support False
            for pp in [False] for ar in [232]
        ]
    else:
        return [
            triton.Config(
            ...
            for bo in [1]  # in tmem
            for SUBTILE in [True]  # doesn't support False
            for pp in [True, False] for ar in [192, 232]
        ]
...
@triton.jit
def _load_tma(bufIdx, phase, empty_bars, full_bars, buffers, desc, offset_1,
              offset_0, num_bytes):
    # producer acquire
...
def gdpa_kernel_tma_ws_blackwell(
    ...
    qk0_buf = tlx.local_alloc((BLOCK_M // 2, HEAD_DIM), tl.float32, 1,
    ...
    p1_buf = tlx.local_alloc((BLOCK_M // 2, HEAD_DIM),
                             ...
                             tlx.storage_kind.tmem,
                             reuse=qk1_buf)
    o0_buf = tlx.local_alloc((BLOCK_M // 2, HEAD_DIM), tl.float32, 1,
                             tlx.storage_kind.tmem)
    o1_buf = tlx.local_alloc((BLOCK_M // 2, HEAD_DIM), tl.float32, 1,
                             tlx.storage_kind.tmem)

    # allocate barriers
    consumer_q0 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_Q,
                                     arrive_count=1)
    consumer_q1 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_Q,
                                     arrive_count=1)
    consumer_release_q0 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_Q,
                                             arrive_count=1)
    consumer_release_q1 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_Q,
                                             arrive_count=1)
    consumer_kv = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_KV,
                                     arrive_count=1)
    consumer_release_kv = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_KV,
                                             arrive_count=1)
    ...
    producer_qk0 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_QK,
                                      arrive_count=1)
    producer_commit_qk0 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_QK,
                                             arrive_count=1)
    producer_qk1 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_QK,
                                      arrive_count=1)
    producer_commit_qk1 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_QK,
                                             arrive_count=1)

    producer_o0 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_O,
                                     arrive_count=1)
    producer_commit_o0 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_O,
                                            arrive_count=1)
    producer_o1 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_O,
                                     arrive_count=1)
    producer_commit_o1 = tlx.alloc_barriers(num_barriers=NUM_BUFFERS_O,
                                            arrive_count=1)
            ...
            for idx in range(0, tiles_per_sm):
                ...
                if start_m * BLOCK_M < qlen:
                    ...
                    for start_n in range(lo, hi, BLOCK_N):
                        ...
                        consumer_qk_view = tlx.local_view(
                            producer_commit_qk0, bufIdx)
                        ...
                        qk_view_2nd = tlx.subslice(qk_view, HEAD_DIM // 2,
                                                   HEAD_DIM // 2)
                        ...
                        consumer_release_qk_view = tlx.local_view(
                            producer_qk0, bufIdx)
        ...
        with tlx.async_task(num_warps=1, registers=24):  # gemm
            ...
            for idx in range(0, tiles_per_sm):
                ...
                if start_m * BLOCK_M < qlen:
                    ...
                    tlx.barrier_wait(consumer_q0_view,
                                     phase_q)  # consumer wait for q0
                    ...
                    tlx.barrier_wait(consumer_kv[bufIdx_k],
                                     phase_k)  # consumer wait for k
                    ...
                    tlx.barrier_wait(consumer_q1_view,
                                     phase_q)  # consumer wait for q1
                    ...
                    tlx.async_dot(
                        ...
                        use_acc=False,
                        mBarriers=[
                            consumer_release_k_view, producer_commit_qk1_view
                        ],
                    )
                    ...
                    tlx.barrier_wait(consumer_kv[bufIdx_v],
                                     phase_v)  # consumer wait for v
                    # need to acquire o0 to make sure epilogue is done, this is needed for each outer loop
                    bufIdx_o_outer, phase_o_outer = _get_bufidx_phase(
                        accum_cnt_outer, NUM_BUFFERS_O)
                    producer_o0_view = tlx.local_view(producer_o0,
                                                      bufIdx_o_outer)
                    producer_o1_view = tlx.local_view(producer_o1,
                                                      bufIdx_o_outer)
                    # tl.device_print("gemm producer_o0", accum_cnt_outer)
                    # tl.device_print("gemm producer_o0_phase", phase_o_outer)
                    # DEBUG_PERF
                    tlx.barrier_wait(producer_o0_view, phase_o_outer
                                     ^ 1)  # producer acquire for o0
                    ...
                    for it in range(BLOCK_N, hi, BLOCK_N):
                        ...
                        consumer_p1_view = tlx.local_view(
                            producer_qk1, bufIdx_qk1)
                        ...
                        consumer_p0_view = tlx.local_view(
                            producer_qk0, bufIdx_qk)
                        # tl.device_print("gemm producer_qk0", accum_cnt_qk)
                    ...
                    release_q0_view = tlx.local_view(consumer_release_q0,
                                                     bufIdx_q)
                    tlx.tcgen05_commit(release_q0_view)
                    release_q1_view = tlx.local_view(consumer_release_q1,
                                                     bufIdx_q)
        ...
        with tlx.async_task(num_warps=1, registers=24):  # load
            ...
            for idx in range(0, tiles_per_sm):
                ...
                if start_m * BLOCK_M < qlen:
                    ...
                    # )
                    # producer acquire
                    q0_empty_view = tlx.local_view(consumer_release_q0,
                                                   q_bufIdx)
                    tlx.barrier_wait(q0_empty_view, q_phase ^ 1)
                    # barrier for producer commit
                    q0_full_view = tlx.local_view(
                        consumer_q0, q_bufIdx)  # full_bars, bufIdx)
                    tlx.barrier_expect_bytes(q0_full_view, BLOCK_M // 2 *
                                             BLOCK_D * 2)  # num_bytes)
                    ...
                    k_bufIdx, k_phase = _get_bufidx_phase(
                        accum_cnt_kv, NUM_BUFFERS_KV)
                    # producer acquire
                    k_empty_view = tlx.local_view(consumer_release_kv,
                                                  k_bufIdx)
                    tlx.barrier_wait(k_empty_view, k_phase)  # ^ 1)
                    # barrier for producer commit
                    k_full_view = tlx.local_view(consumer_kv, k_bufIdx)
                    tlx.barrier_expect_bytes(k_full_view, BLOCK_N * BLOCK_D *
                                             2)  # num_bytes)
                    ...
                    # producer acquire
                    q1_empty_view = tlx.local_view(consumer_release_q1,
                                                   q_bufIdx)
                    tlx.barrier_wait(q1_empty_view, q_phase ^ 1)
                    # barrier for producer commit
                    q1_full_view = tlx.local_view(consumer_q1, q_bufIdx)
                    tlx.barrier_expect_bytes(q1_full_view, BLOCK_M // 2 *
                                             BLOCK_D * 2)  # num_bytes)
                    ...
                    for start_n in range(BLOCK_N, hi, BLOCK_N):
                        ...
                        # barrier for producer commit
                        k_full_view = tlx.local_view(consumer_kv, k_bufIdx)
                        tlx.barrier_expect_bytes(k_full_view, BLOCK_N *
                                                 BLOCK_D * 2)  # num_bytes)
                        k_view = tlx.local_view(kv_buf, k_bufIdx)
                        tlx.async_descriptor_load(
                            k_desc,
...
def gdpa_forward_tlx(
    ...
    if broadcast_q:
        ...
        BATCH = (query_offset.size(0) //
                 2 if use_start_end_offsets else query_offset.size(0) - 1)
...
def generate_jagged_data(
    ...
    x_offsets = torch.cat(
        [torch.IntTensor([0]).to(device),
         num_objects.cumsum(dim=0)], dim=0)
    q_offsets = x_offsets
    ...
    if dense_q:
        ...
        grad_o = torch.rand(B * dense_q_len, H, D, device=device,
                            dtype=dtype) * 0.01
        if not broadcast_q:
            q_weights = torch.rand(B * dense_q_len,
                                   H,
                                   D,
                                   device=device,
                                   dtype=dtype)
            num_objects_q = torch.tensor([dense_q_len] * B,
                                         device=device,
                                         dtype=torch.int32)
            q_offsets = torch.cat(
                [torch.IntTensor([0]).to(device),
                 num_objects_q.cumsum(dim=0)],
                dim=0)
        else:
            q_weights = torch.rand(dense_q_len,
                                   H,
                                   D,
                                   device=device,
                                   dtype=dtype)
            num_objects_q = torch.tensor([dense_q_len] * B,
                                         device=device,
                                         dtype=torch.int32)
            q_offsets = torch.tensor([0, dense_q_len],
                                     dtype=torch.int,
                                     device=device)
    ...
    return {
        ...
        attn_offsets,
        "max_seq_len":
        max(max_M, dense_q_len if dense_q and dense_q_len else 0,
            dff if dff else 0),
        "max_seq_len_q":
        dense_q_len if dense_q and dense_q_len else max_M,
        "max_seq_len_k":
        dff if dff else max_M,
        "dense_q_len":
        dense_q_len if dense_q else None,
        "bias":
        bias_tensor,
        "invalid_attn_mask":
        invalid_attn_mask.contiguous(),
        "do":
        grad_o,
        "mask_lower_triangle":
        False,
...
def get_tlx_gdpa_fn(config):
    ...
    jagged_data["seq_index"] = torch.argsort(jagged_data["num_objects_q"],
                                             descending=True).contiguous()
