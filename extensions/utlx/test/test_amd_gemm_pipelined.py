...
from conftest import tlx, DEVICE, is_hip, is_cuda, is_hip_cdna2
    ...
    configs = [
        triton.Config({
            ...
            'waves_per_eu': waves_per_eu
                      ...
                      num_warps=num_warps) for block_m in [32, 64, 128, 256]
        for block_n in [32, 64, 128, 256] for block_k in [32, 64, 128]
        for group_size_m in [1, 2, 4, 8] for waves_per_eu in [0, 2, 4]
        for num_stages in [2] for num_warps in [4, 8]
        for mat_dim_non_k in [16] for kpack in [1, 2]
    ]
    return configs
...
def matmul_kernel_pipelined_mi300(
    ...
    buffers_A = tlx.local_alloc((BLOCK_SIZE_M, BLOCK_SIZE_K),
                                tlx.dtype_of(a_ptr), NUM_STAGES - 1)
    buffers_B = tlx.local_alloc((BLOCK_SIZE_K, BLOCK_SIZE_N),
                                tlx.dtype_of(b_ptr), NUM_STAGES - 1)
    ...
    # Epilogue
    for k in tl.range(K_ITERS - (NUM_STAGES - 1),
                      K_ITERS,
