...
static std::optional<int64_t> extractConstInt(mlir::Value v) {
  if (auto constIntOp = mlir::dyn_cast_or_null<mlir::arith::ConstantIntOp>(
          v.getDefiningOp()))
...
/// Layout: shape = [rank values], order = [rank values], then fp4Padded, swizzled
/// Total operands = 1(result) + 1(src) + rank(shape) + rank(order) + 2(flags)
/// The rank is inferred: (numOperands - 4) / 2
void utlx::createRequireNvMmaSharedLayout(TritonOpBuilder &self,
  ...
  size_t numShapeOrder = operands.size() - 4; // minus result, src, fp4, swizzled
  ...
  if (swizzled) {
    encoding = ttg::NVMMASharedEncodingAttr::get(
        context, shape, order, cgaLayout,
        srcType.getElementType(), fp4Padded);
  } else {
    encoding = ttg::NVMMASharedEncodingAttr::get(
...
void utlx::createRequireNvMmaLayout(TritonOpBuilder &self,
  ...
  else if (auto tensorTy = mlir::dyn_cast<mlir::RankedTensorType>(opndA.getType()))
  ...
  auto instrShape = mlir::mmaVersionToInstrShape(
...
/// parent_carrier: a Value whose type has the parent encoding (from require_nv_mma_layout)
void utlx::createRequireDotOperandLayout(TritonOpBuilder &self,
  ...
  if (auto tensorTy = mlir::dyn_cast<mlir::RankedTensorType>(parentCarrier.getType()))
  ...
  auto encoding = ttg::DotOperandEncodingAttr::get(
      context, opIdx, parentEnc, opndType.getElementType());
...
void utlx::createRequireTensorMemoryLayout(TritonOpBuilder &self,
  ...
  auto encoding = ttng::TensorMemoryEncodingAttr::get(
      context,
      static_cast<unsigned>(*blockMVal),
      static_cast<unsigned>(*blockNVal),
      static_cast<unsigned>(*colStrideVal),
...
void utlx::createAsyncLoad(TritonOpBuilder &self,
  ...
  if (useBulk) {
    ...
    // Non-bulk: operands[1]=src, operands[2]=result, then optional mask/other, then useBulk=0
    llvm::SmallVector<mlir::Value> opOperands = {src, result};
...
void utlx::createGlobalScratchAlloc(TritonOpBuilder &self,
  ...
  auto nbytesAttr = self.getBuilder().getI32IntegerAttr(
      static_cast<int32_t>(*nbytesVal));
  auto alignmentAttr = self.getBuilder().getI32IntegerAttr(
      static_cast<int32_t>(*alignmentVal));

  auto *op = createRuntimeOp(self.getBuilder(), self.getLastLoc(),
                              "triton_gpu.global_scratch_alloc",
                              {ptrType}, {},
...
/// utlx_make_dummy_register_layout(result_slot, shape..., type_carrier, tmemCompatible)
/// Creates a DummyRegisterLayoutAttr and returns it as a RequireLayoutOp result.
/// NOTE: This returns an attribute-carrying value, not a real layout transform.
/// It's used only to create a type carrier for create_tmem_load/create_tmem_store.
void utlx::createMakeDummyRegisterLayout(TritonOpBuilder &self,
  ...
  auto *context = self.getBuilder().getContext();
  auto encoding = tlx::DummyRegisterLayoutAttr::get(
      context, shape, elementType, tmemCompatible);
...
/// Applies the layout encoding from layout_carrier's type to src via RequireLayoutOp.
void utlx::createRequireWithLayoutCarrier(TritonOpBuilder &self,
  ...
  mlir::Attribute encoding;
  if (auto tensorTy = mlir::dyn_cast<mlir::RankedTensorType>(layoutCarrier.getType()))
    encoding = tensorTy.getEncoding();
  else if (auto memTy = mlir::dyn_cast<ttg::MemDescType>(layoutCarrier.getType()))
  ...
  if (auto tensorTy = mlir::dyn_cast<mlir::RankedTensorType>(src.getType())) {
    ...
    auto newType = ttg::MemDescType::get(
        memTy.getShape(), memTy.getElementType(), encoding,
...
void utlx::createAllocClcResponses(TritonOpBuilder &self,
  ...
  auto memDescType = ttg::MemDescType::get(
      {numResponses}, i128Type, encoding, memorySpace, /*mutableMemory=*/true);
...
void utlx::createClcQuery(TritonOpBuilder &self,
  ...
  auto *queryOp = createRuntimeOp(builder, loc,
                                   "triton_nvidia_gpu.clc_query_cancel",
                                   {i32Ty}, {operands[1]});
  ...
  auto *rankOp = createRuntimeOp(builder, loc,
                                  "nvgpu.cluster_id", {i32Ty}, {});
  ...
  mlir::Value offset = mlir::arith::AddIOp::create(builder, loc, tileId, ctaRank);
  mlir::Value result = mlir::arith::SelectOp::create(builder, loc, isNegOne, tileId, offset);
...
void utlx::createThreadId(TritonOpBuilder &self,
  ...
  auto threadId = mlir::gpu::ThreadIdOp::create(builder, loc, indexType, dims[axis]);
  auto i32Val = mlir::arith::IndexCastOp::create(builder, loc,
                                                   builder.getI32Type(),
                                                   threadId);
  operands[0] = i32Val;
}
