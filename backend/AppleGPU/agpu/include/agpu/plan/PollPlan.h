...
struct PollFacts {
  ...
  // A timeout poll tests once and reports; a plain one spins. Apple GPUs
  // expose no in-shader clock, so the budget itself is never measured: the
  // op asks for one load even at zero, which is what a spent budget gets.
