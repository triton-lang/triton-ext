...
LogicalResult LoopBisect::bisect() {
  ...
  if (cmpMap.size() >= 1) {
    ...
    if (*step != 1) {
      // midp += (midp % step) ? 1 : 0
      auto zero = arith::ConstantIntOp::create(b, loc, 0, 32);
      auto one = arith::ConstantIntOp::create(b, loc, 1, 32);
      auto rem = arith::RemUIOp::create(b, loc, midp, forOp.getStep());
      auto rcmp =
          arith::CmpIOp::create(b, loc, arith::CmpIPredicate::ne, rem, zero);
      auto sel = arith::SelectOp::create(b, loc, rcmp, one, zero);
      midp = arith::AddIOp::create(b, loc, midp, sel);
    }
...
#define PLUGIN_NAME "SELoopSplit"
